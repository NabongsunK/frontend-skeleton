...
const BoardDetail = function (props) {
  ...
  const { id } = useParams();
  const { data, error } = useQuery(['boards', id], () => axios.get(`/boards/${id}?delay=900`), {
    suspense: true,
      ...
      { data && <>        
        <div className="col-lg-8 entries">
          <article className="entry entry-single">
            ...
            <div className="entry-content">
              <a href={`http://localhost:30443/uploads/${item.attachFile}`} download target="_blank" rel="noreferrer">{item.attachFileName}</a>
            </div>
